...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const isMember = await prisma.clubMember.findUnique({ where: { clubId_playerId: { clubId: params.id, playerId } } });
  if (!isMember && club.managerId !== playerId && !isSiteAdmin) return new Response("Non autorisé", { status: 403 });
