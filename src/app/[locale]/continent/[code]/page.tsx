    ...
    <div className="continent-page">
      <div className="section-title">
        <h1>{continentNames[code] ?? code}</h1>
          <Link className="ghost" href="/">{t("btn_back")}</Link>
        </div>
        ...
        <section className="section" style={{ marginTop: 32 }}>
          <div className="section-header">
            <div>
              ...
              <p>{t("tournaments_subtitle")}</p>
            </div>
            <Link className="primary" href="/tournament/new">{t("tournaments_new")}</Link>
          </div>
          {tournaments.length > 0 ? (
            ...
            <div className="empty-state"><p>{t("tournaments_empty")}</p></div>
          )}
        </section>
        ...
        <section className="section">
          <div className="section-header">
            <div>
              <h2>{t("clubs_title")}</h2>
              <p>{clubs.length === 1 ? tt("count_singular") : tt("count_plural")} — {clubs.length}</p>
            </div>
            <Link className="primary" href="/club/new">{t("clubs_add")}</Link>
          </div>
                  ...
                  <div key={country}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                      ...
                      <Link href={`/continent/${code}/${encodeURIComponent(country)}`} className="ghost" style={{ fontSize: 13 }}>
                        {t("clubs_see_all", { count: countryMap[country] ?? 0 })}
                      </Link>
                    </div>
