...
export default async function TournamentPage({
  ...
  return (
    <div className="tournament-page">
      ...
      {tab === "kiosque_j1" && (isTestMode
        ? <div className="panel" style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>🧪 {t("test_mode_hidden")}</div>
        : <PoolTables pools={(tournament.pools ?? []).filter((p: any) => p.name === "Pool A" || p.name === "Pool B")} matches={(tournament.matches ?? []).filter((m: any) => m.phase === "KIOSQUE_POOL")} tournamentId={tournament.id} scoringSystem={tournament.scoringSystem} isLive={tournament.status === "LIVE"} poolRounds={null} teamsWithPlayers={tournament.teams as any} />
      )}

      {tab === "kiosque_top4" && (isTestMode
        ? <div className="panel" style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>🧪 {t("test_mode_hidden")}</div>
        : <PoolTables pools={(tournament.pools ?? []).filter((p: any) => p.name === "Top 4")} matches={(tournament.matches ?? []).filter((m: any) => m.phase === "KIOSQUE_TOP4")} tournamentId={tournament.id} scoringSystem={tournament.scoringSystem} isLive={tournament.status === "LIVE"} poolRounds={null} teamsWithPlayers={tournament.teams as any} />
      )}

      {tab === "kiosque_bot12" && (isTestMode
        ? <div className="panel" style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>🧪 {t("test_mode_hidden")}</div>
        : <PoolTables pools={(tournament.pools ?? []).filter((p: any) => p.name === "Bottom 12")} matches={(tournament.matches ?? []).filter((m: any) => m.phase === "KIOSQUE_BOTTOM12")} tournamentId={tournament.id} scoringSystem={tournament.scoringSystem} isLive={tournament.status === "LIVE"} poolRounds={null} teamsWithPlayers={tournament.teams as any} />
      )}

      {tab === "kiosque_se" && (isTestMode
        ? <div className="panel" style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>🧪 {t("test_mode_hidden")}</div>
        : (() => {
      ...
      {tab === "bracket" && !isTestMode && isBigApple && (() => {
        const seMatches = tournament.matches.filter((m: any) => m.phase === "BIG_APPLE_SE");
        const bracketTeams = tournament.teams.filter((t) => t.selected).map((team) => ({ id: team.id, name: team.name, bracketNumber: team.seed }));
        ...
        return (
          <div>
            <BracketView matches={seMatches as any} tournamentId={tournament.id} teams={bracketTeams} isOrganizer={isOrga} isLive={tournament.status === "LIVE"} />
          </div>
        );
      })()}
      ...
      {tab === "bracket" && !isTestMode && !isGraz && !isMtpOpen && !isBigApple && !isPipeline && (() => {
            ...
            <>
              ...
              {isOrga && tournament.crossPool && (
                <p className="meta" style={{ marginBottom: 12, fontSize: 12 }}>
                  {t("bracket_cross_pool_managed_hint")}
                </p>
              )}
        ...
        const canLaunch = isOrga && isLaunched && !tournament.crossPool && (swissDone || poolDone || (!isPool && tournament.saturdayFormat !== "SWISS"));
        return (
      ...
      {/* ── SPLIT_SE: R1 ── */}
      {tab === "r1" && (() => {
        const bracketTeams = tournament.teams.filter((t: any) => t.selected).map((team: any) => ({
          id: team.id,
          name: team.name,
          bracketNumber: team.seed,
        }));
        if (splitSeR1Matches.length > 0) {
          return <BracketView matches={splitSeR1Matches} tournamentId={tournament.id} teams={bracketTeams} isOrganizer={isOrga} isLive={tournament.status === "LIVE"} />;
        }
        return <div className="panel" style={{ textAlign: "center", padding: 48 }}><p className="meta">{t("no_matches_for_day")}</p></div>;
      })()}
      ...
      {tab === "winners" && (() => {
        ...
        if (splitSeWinnersMatches.length > 0) {
          return <BracketView matches={splitSeWinnersMatches} tournamentId={tournament.id} teams={bracketTeams} isOrganizer={isOrga} isLive={tournament.status === "LIVE"} />;
        }
        return <div className="panel" style={{ textAlign: "center", padding: 48 }}><p className="meta">{t("no_matches_for_day")}</p></div>;
      })()}
      ...
      {tab === "losers" && (() => {
        ...
        if (splitSeLosersMatches.length > 0) {
          return <BracketView matches={splitSeLosersMatches} tournamentId={tournament.id} teams={bracketTeams} isOrganizer={isOrga} isLive={tournament.status === "LIVE"} />;
        }
        return <div className="panel" style={{ textAlign: "center", padding: 48 }}><p className="meta">{t("no_matches_for_day")}</p></div>;
      })()}
      ...
      {tab === "live" && (
        <LiveTabView
          ...
          court2Embed={court2Embed}
          multiplexEmbed={multiplexEmbed}
